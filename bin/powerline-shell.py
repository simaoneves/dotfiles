...
class DefaultColor:
    ...
    HOME_BG = 209  # orange
    HOME_FG = 0  # white
    PATH_BG = 147  # dark grey
    PATH_FG = 0  # light grey
    ...
    REPO_CLEAN_BG = 150  # a light green color
    REPO_CLEAN_FG = 0  # black
    REPO_DIRTY_BG = 222  # pink/red
...
def parse_git_stats(status):
    stats = {'untracked': 0, 'notstaged': 0, 'staged': 0, 'conflicted': 0,'all': 0}
...
def add_git_segment(powerline):
    ...
    def _add_simple(_dict, _key, fg, bg):
        if _dict[_key]:
            _str = u' {} '.format(GIT_SYMBOLS[_key])
            powerline.append(_str, fg, bg)

    # if branch_info:
    #     _add_simple(branch_info, 'ahead', fg, bg)
    #     _add_simple(branch_info, 'behind', Color.GIT_BEHIND_FG, Color.GIT_BEHIND_BG)
    ...
    # _add(stats, 'all', Color.GIT_CONFLICTED_FG, Color.GIT_CONFLICTED_BG)
